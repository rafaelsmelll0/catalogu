...
import { searchMovies, getMovieDetails, getPosterUrl, getBackdropUrl } from './tmdb.js'
...
interface V2Genre  { name: string }
interface V2Tag    { name: string }
interface V2Person { name: string; role: string }

function getV2Data(v2db: Database.Database, mediaId: number) {
  const genres = v2db.prepare(`
    SELECT g.name FROM genres g
    JOIN media_genres_link l ON l.genre_id = g.id
    WHERE l.media_id = ?
  `).all(mediaId) as V2Genre[]

  const tags = v2db.prepare(`
    SELECT t.name FROM tags t
    JOIN media_tags_link l ON l.tag_id = t.id
    WHERE l.media_id = ?
  `).all(mediaId) as V2Tag[]

  const people = v2db.prepare(`
    SELECT p.name, l.role FROM people p
    JOIN media_people_link l ON l.person_id = p.id
    WHERE l.media_id = ?
  `).all(mediaId) as V2Person[]

  return { genres, tags, people }
}

function alreadyExists(title: string, releaseYear: string | null, tmdbId: number | null): boolean {
...
function insertMedia(
  ...
  for (const g of genres) {
    let row = db.prepare('SELECT id FROM genres WHERE name = ?').get(g) as { id: number } | undefined
    if (!row) {
  ...
  for (const t of tags) {
    let row = db.prepare('SELECT id FROM tags WHERE name = ?').get(t) as { id: number } | undefined
    if (!row) {
  ...
  for (const c of cast) people.push({ name: c, role: 'actor' })
...
export async function importFromV2Db(
  dbPath:     string,
  onProgress: (p: ImportProgress) => void,
): Promise<ImportResult> {
  const v2db = new Database(dbPath, { readonly: true })

  const v2medias = v2db.prepare(`
    SELECT * FROM media WHERE tipo = 'filme' ORDER BY title
  `).all() as V2Media[]

  const importResult: ImportResult = { imported: 0, skipped: 0, failed: 0, errors: [] }
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      const results = await searchMovies(v2.title)
      let matched = null
      ...
      if (matched) {
        const details      = await getMovieDetails(matched.id)
        const posterUrl    = getPosterUrl(details.poster_path, 'w500')
        const backdropUrl  = details.backdrop_path
          ? getBackdropUrl(details.backdrop_path, 'w1280')
          : null
        const tmdbDir    = details.credits?.crew?.find((c: { job: string; name: string }) => c.job === 'Director')?.name ?? null
        const tmdbCast   = (details.credits?.cast ?? []).slice(0, 5).map((c: { name: string }) => c.name)
        const tmdbGenres = (details.genres ?? []).map((g: { name: string }) => g.name)

        insertMedia(
          v2, matched.id, posterUrl, backdropUrl,
          cast.length > 0       ? cast       : tmdbCast,
          director               ?? tmdbDir,
          genreNames.length > 0  ? genreNames : tmdbGenres,
          tagNames,
        )
